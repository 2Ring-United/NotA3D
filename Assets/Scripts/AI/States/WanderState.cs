...
public class WanderState : AIState
{
        ...
        else
        {
            ...
            if (!controller.GetNavMeshAgent().pathPending && controller.GetNavMeshAgent().remainingDistance <= controller.GetNavMeshAgent().stoppingDistance)
            {
                if (!controller.GetNavMeshAgent().hasPath || controller.GetNavMeshAgent().velocity.sqrMagnitude <= 0.2f)
                {
                    ...
                    if (!controller.GetAnimator().GetNextAnimatorStateInfo(0).IsName(idleAnimation.ToString()))
                    {
                        controller.GetAnimator().CrossFade(idleAnimation.ToString(), 0.1f);
                    }
                }
            }
            /*
    ...
    private void SetRandomDestination(AIController controller)
    {
        ...
        if (!controller.GetAnimator().GetNextAnimatorStateInfo(0).IsName(walkAniamtion.ToString()))
        {
            controller.GetAnimator().CrossFade(walkAniamtion.ToString(), 0.1f);
        }
    }
    ...
    public override bool CanChangeToState(AIController controller)
    {
        ...
        return distanceToTarget > controller.characterStats.AttackRange || controller.GetTarget() == null;
    }
}
